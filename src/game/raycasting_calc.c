...
/*   Updated: 2025/02/05 06:57:42 by anoteris         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_step_and_sideDist(t_raycast *raycast)
{
	...
	{
		raycast->step->x = -1;
		raycast->sideDist->x = (raycast->pos->x - raycast->map->x) * raycast->deltaDist->x;
	}
	else
	{
		raycast->step->x = 1;
		raycast->sideDist->x = (raycast->map->x + 1.0 - raycast->pos->x) * raycast->deltaDist->x;
	}
	if(raycast->rayDir->y < 0)
	{
		raycast->step->y = -1;
		raycast->sideDist->y = (raycast->pos->y - raycast->map->y) * raycast->deltaDist->y;
	}
	else
	{
		raycast->step->y = 1;
		raycast->sideDist->y = (raycast->map->y + 1.0 - raycast->pos->y) * raycast->deltaDist->y;
	}
}
...
int	calc_texX(t_raycast *raycast, mlx_image_t *img)
{
	...
	if((raycast->side == EA || raycast->side == WE) && raycast->rayDir->x > 0)
		texX = img->width - texX - 1;
	if((raycast->side == NO || raycast->side == SO) && raycast->rayDir->y < 0)
		texX = img->width - texX - 1;
	return (texX);
}
